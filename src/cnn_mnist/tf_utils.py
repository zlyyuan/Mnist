...
def preprocess_mnist_data(data_set):
    x, y = data_set
...
def cnn_random_mini_batches(X, Y, mini_batch_size=64, seed=0):
    """
...
def random_mini_batches(X, Y, mini_batch_size = 64, seed = 0):
    ...
    X = X.eval()            #convert X from tensor to np.array
    Y = Y.eval()
    ...
    for k in range(0, num_complete_minibatches):
        ...
        mini_batch = (np.transpose(mini_batch_X), np.transpose(mini_batch_Y))
        mini_batches.append(mini_batch)
